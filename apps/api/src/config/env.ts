...
export const envSchema = z
  .object({
    ...
    GONKA_MAX_RETRIES: z.coerce.number().int().min(0).max(5).default(2),
    GONKA_RETRY_BASE_MS: z.coerce.number().int().min(1000).default(30000),
    TAVILY_API_KEY: z.string().optional(),
    TAVILY_SEARCH_DEPTH: z.enum(['basic', 'advanced']).default('advanced'),
    TAVILY_MAX_RESULTS_PER_CLAIM: z.coerce.number().int().min(1).max(5).default(5),
